...
<<<<<<< HEAD
Projects = 'All' #'FAL' #  #c('CGR','FAL')
=======
Projects = 'FAL' # 'All' #c('CGR','FAL')
...
source(file.path(WriteDir,'data','QGT_elvsFrom_w2_con_2026-01-06.r'))
...
>>>>>>> fa19c5f11a312acbfcca8860bc583de252d3b5e9
  ...
  tsites <- data.frame(BCL = 'BCLO',CGR = 'CGRO',FOS = 'SSFO',DEX = 'DEXO',HCR = 'HCRO')
  ...
  xlmExcL[['Elev-Temp']] <- calcThreshMon(DataType = 'Elev-Forebay',TVals = 'WtempControl')
  ...
  if(any(grepl(Proj,names(tsites)))){
    ...
    # xlaStlhdEggEmrg <-EstimateEmergence(xld[grepl(tsites,xld$name),],
    #                   spawnDay = c(244,263,274),
    #                   hatchValue = 1150 # Ask Rich for these values for Steelhead
    #                   )
    ...
    print(paste("Skipping Estimated Emergence; No temperature data at",tsites))
    xlaEggEmrg <- NULL
  }
...
=======
      dplyr::select('DateTime',contains('Opening'),contains('Flow-Gen'),contains('Flow-Spill'),contains('Flow-Out')) |>
>>>>>>> fa19c5f11a312acbfcca8860bc583de252d3b5e9
...
<<<<<<< HEAD
         ...
         #name = gsub('.Opening','.Open',name)) |>
  separate_wider_delim(name,delim='-',names = c('name','gate')) |>
  mutate(data = 'CWMS',
         param = if_else(grepl('Flow',name) | grepl('Flow',gate),'Flow',if_else(grepl('Open',gate),'Opn','NA')),
         name = gsub('.Flow','',name),
         gate = gsub('.Flow|.Opening|.Open','',gate))
...
=======
# Left off here!!! Go through each project and send flow data for each outlet to Will.
>>>>>>> fa19c5f11a312acbfcca8860bc583de252d3b5e9
