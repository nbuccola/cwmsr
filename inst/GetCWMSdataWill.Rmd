...
for(i in whichProj){
  ...
  xlaExc <- calcThreshAnn(xlmExc |> filter(!grepl('DysWSELVExc',ExcType))) |>
    full_join(calcThreshAnn(xlmExc |> filter(grepl('DysWSELVExc',ExcType)),months = 4:9))
...
# Flow Metrics
MultiYrQPlots(SaveName = '2025Ops')
OpsExc
