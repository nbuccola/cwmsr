...
FishCrit = jsonlite::fromJSON(file.path(WriteDir,"data","FishCriteria.json"))[['Criteria']]
source(file.path(LocalWd,'R','temp_calcs.r'))
FishCrit_ts <- ReformatFishCrit(FishCrit)
...
tsites[1,] <- paste0(tsites[1,],'.Temp-Water')
TDGsites <- data.frame(DET = 'DET',BCL = 'BCLO',
                       GPR = 'GPRO',FOS = 'SSFO',
                       CGR = 'CGRO',
                       HCR = 'HCRO',FAL = 'FALO',LOP = 'LPQO',DEX = 'DEXO')
TDGsites[1,] <- paste0(TDGsites[1,],'.%-Saturation-TDG')
...
for(i in whichProj){
  ...
  if(any(grepl(Proj,names(tsites))) |
     any(grepl(Proj,names(TDGsites)))){
    # Calculate days of exceedance for Temperature variables
    # Check for temperature data and sites of interest
    source(file.path(LocalWd,'R','temp_calcs.r'))
    xlmExcL <- xlExcDtsL <- list(NA)
    xlmExcL[['Temp']] <- calcThreshMon(DataType = 'Temp-Water',TVals = c(16,18,20,'TTargPath','FishCrit'))
    xlExcDtsL[['Temp']] <- CalcDatesOfExc(DataType = 'Temp-Water',TVals = c(16,18,20))
    #xlExcDtsL[['Temp']] <- CalcDatesOfExc(DataType = 'Temp-Water',TVals = 'FishCrit')
    xlmExcL[['TDG']] <- calcThreshMon(DataType = 'TDG',TVals = c(110,120)) 
    #xlExcDtsL[['TDG']] <-  CalcDatesOfExc(DataType = 'TDG',TVals = c(110,120)) # Not implemented yet; only applies to Temp-Water
    ...
    xlExcDtsL <- Filter(Negate(is.logical), xlExcDtsL)
    xlExcDts <- purrr::reduce(xlExcDtsL,full_join)
  ...
  save(Proj,OutletI,xw,xld,xldSum,xlm,xlmExc,xla,xlaExc,xlaEggEmrg,xlExcDts,#cwms_paths,
       file = file.path(RdataDir,gsub('_OpsRaw','_OpsProcessed',cnfg[[i]][['dataFileName']])))
  
  rm(xw,xld,xldSum,xlm,xlmExc,xlmExcL,xla,xlaExc,xlaEggEmrg,xlExcDts,Proj)
}
...
GtsCWMS <- Gts |>
  ...
  mutate(data = 'CWMS',
         param = if_else(grepl('Flow',gate),'Flow',if_else(grepl('Opening',gate),'Opn','NA')),
         gate = gsub('SB','SWG',gsub('.Flow|.Opening','',gate)))
...
unique(GtOpnWV$gate)
summary(GtOpnWV |> 
         filter(grepl('DET',name),!grepl('Spill|Out|Total|Gen|RG',gate)) |>
...
for(i in whichProj){
  ...
  rm(xw,xld,xldSum,xlm,xlmExc,xla,xlaExc,xlaEggEmrg,xlExcDts,Proj)
}
DataList[['xld']] <- Filter(Negate(is.null), DataList[['xld']])
DataList[['Exc']] <- Filter(Negate(is.null), DataList[['Exc']])
DataList[['ExcDts']] <- Filter(Negate(is.null), DataList[['ExcDts']])
DataList[['Stats']] <- Filter(Negate(is.null), DataList[['Stats']])
DataList[['EggEmrg']] <- Filter(Negate(is.null), DataList[['EggEmrg']])
OpsXld <- purrr::reduce(DataList[['xld']],full_join) 
OpsExc <- purrr::reduce(DataList[['Exc']],full_join) 
OpsExcDts <- purrr::reduce(DataList[['ExcDts']],full_join) 
...
clrs<- c('2021'="#999999",'2022'= "#E69F00",'2023'= "#56B4E9",'2024'= "#009E73", '2025'="#D55E00") #, "#CC79A7", "#0072B2") #"#F0E442",
...
for(i in 1:length(sb)){
  # Prob with 2025Ops_SFMck_AnnTTargAbvBlwArrows numbers are too high!!!
  # Fix TDG monthly/annual plot order for MFWill
  # Add TDG to DET-BCL monthly/annual?
...
names(PlotDataList) <- sb
PlotData <- as.data.frame(do.call(rbind, PlotDataList))


```

Write a spreadsheet with data metrics
...
WTxlsxList1 <- WTxlsxList2 <- list()

WTxlsxList1$OpsEggEmrg = OpsEggEmrg |>
      ...
      select(-nmiss) |>
      pivot_wider(names_from = spawnDay,id_cols = c(name,Year,species),values_from = c('atu'))

WTxlsxList1$OpsExc = OpsExc |>
    mutate(
...
WTxlsxList1$OpsStats = OpsStats |>
    mutate(Month = factor(Month,levels = c(1:12,'Annual'),ordered=T),
           name = gsub('.0|.1Day|.COORD_Fisheries_|.USACE_Temporary_|.NMFS_USFWS_ODFW_| ','',
                       gsub('.Fisheries ','.',name)),
           Year = as.numeric(Year)) 

WTxlsxList1$OpsExcDts = OpsExcDts |>
    mutate(name = gsub('.0|.1Day|.COORD_Fisheries_|.USACE_Temporary_|.NMFS_USFWS_ODFW_| ','',
                       gsub('.Fisheries ','.',name)),
           ExcMeanVal = round(ExcMeanVal,1),
           ExcMaxVal = round(ExcMaxVal,1),
           Year = as.numeric(format(ExcIDstrt,'%Y'))) 

# Make a table with biologic criteria and timing
WTxlsxList1$BioCrit <- FishCrit |>
  mutate(acute_F = round(c2f(acute),1),
         chronic_F = round(c2f(chronic),1)) |>
  select(-color) |>
  relocate(threshold,.after = last_col())

openxlsx::write.xlsx(WTxlsxList1,
                     file = file.path(RdataDir,paste(SaveName,'AllMetrics.xlsx',sep = '_')),
